package com.zvosframework.schedule.central.schedule;
...
public class JobScheduleTest {
    ...
    private void lockTest(String threadName) {
        ...
        TransactionStatus transactionStatus = XxlJobCentralBootstrap.getInstance().getTransactionManager().getTransaction(new DefaultTransactionDefinition());
        try {
            String lockedRecord = XxlJobCentralBootstrap.getInstance().getXxlJobLockMapper().scheduleLock(); // for update
            ...
            XxlJobCentralBootstrap.getInstance().getTransactionManager().commit(transactionStatus);
        }
    }
}
