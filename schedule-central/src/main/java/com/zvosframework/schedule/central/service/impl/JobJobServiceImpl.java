...
import com.zvosframework.schedule.central.scheduler.route.ExecutorRouteStrategyEnum;
import com.zvosframework.schedule.central.scheduler.thread.JobScheduleHelper;
...
import com.zvosframework.schedule.central.scheduler.type.strategy.TimeRangeConfig;
...
import com.zvosframework.schedule.central.util.JobGroupPermissionUtil;
...
public class JobServiceImpl implements JobService {
	...
	public Response<PageModel<JobInfo>> pageList(int offset, int pagesize, int jobGroup, int triggerStatus, String jobDesc, String executorHandler, String author) {
		...
		// page list
		List<JobInfo> list = xxlJobInfoMapper.pageList(offset, pagesize, jobGroup, triggerStatus, jobDesc, executorHandler, author);
		...
		if (StringTool.isNotBlank(jobInfo.getChildJobId())) {
			...
			for (String childJobIdItem: childJobIds) {
				if (StringTool.isNotBlank(childJobIdItem) && StringTool.isNumeric(childJobIdItem)) {
					JobInfo childJobInfo = xxlJobInfoMapper.loadById(Integer.parseInt(childJobIdItem));
	...
	public Response<String> update(JobInfo jobInfo, LoginInfo loginInfo) {
		...
		if (StringTool.isNotBlank(jobInfo.getChildJobId())) {
			...
			for (String childJobIdItem: childJobIds) {
				if (StringTool.isNotBlank(childJobIdItem) && StringTool.isNumeric(childJobIdItem)) {
					...
					// valid child
					JobInfo childJobInfo = xxlJobInfoMapper.loadById(childJobId);
					if (childJobInfo==null) {
						return Response.ofFail (
								MessageFormat.format((I18nUtil.getString("jobinfo_field_childJobId")+"({0})"+I18nUtil.getString("system_not_found")), childJobIdItem));
					}
		...
		// group valid
		JobGroup jobGroup = xxlJobGroupMapper.load(jobInfo.getJobGroup());
		if (jobGroup == null) {
			return Response.ofFail ( (I18nUtil.getString("jobinfo_field_jobgroup")+I18nUtil.getString("system_invalid")) );
		}

		// stage job info
		JobInfo exists_jobInfo = xxlJobInfoMapper.loadById(jobInfo.getId());
	...
	public Response<String> remove(int id, LoginInfo loginInfo) {
		// valid job
		JobInfo xxlJobInfo = xxlJobInfoMapper.loadById(id);
		if (xxlJobInfo == null) {
			return Response.ofSuccess();
		}
	...
	public Response<String> trigger(LoginInfo loginInfo, int jobId, String executorParam, String addressList) {
		// valid job
		JobInfo xxlJobInfo = xxlJobInfoMapper.loadById(jobId);
		if (xxlJobInfo == null) {
			return Response.ofFail(I18nUtil.getString("jobinfo_glue_jobid_invalid"));
		}
		...
		JobCentralBootstrap.getInstance().getJobTriggerPoolHelper().trigger(jobId, TriggerTypeEnum.MANUAL, -1, null, executorParam, addressList);
	...
	public Map<String, Object> dashboardInfo() {
		...
		JobLogReport xxlJobLogReport = xxlJobLogReportMapper.queryLogReportTotal();
		...
		List<JobGroup> groupList = xxlJobGroupMapper.findAll();
	...
	public Response<Map<String, Object>> chartInfo(Date startDate, Date endDate) {
		...
		List<JobLogReport> logReportList = xxlJobLogReportMapper.queryLogReport(startDate, endDate);
