package com.zvosframework.schedule.central.scheduler.alarm.impl;

import com.zvosframework.schedule.central.scheduler.alarm.JobAlarm;
...
public class EmailJobAlarm implements JobAlarm {
    ...
    public boolean doAlarm(JobInfo info, JobLog jobLog){
        ...
        if (info!=null && info.getAlarmEmail()!=null && !info.getAlarmEmail().trim().isEmpty()) {
            ...
            // email info
            JobGroup group = JobCentralBootstrap.getInstance().getJobGroupMapper().load(Integer.valueOf(info.getJobGroup()));
            String personal = I18nUtil.getString("central_name_full");
            ...
            for (String email: emailSet) {
                ...
                // make mail
                try {
                    MimeMessage mimeMessage = JobCentralBootstrap.getInstance().getMailSender().createMimeMessage();

                    MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true);
                    helper.setFrom(JobCentralBootstrap.getInstance().getEmailFrom(), personal);
                    helper.setTo(email);
                    helper.setSubject(title);
                    helper.setText(content, true);

                    JobCentralBootstrap.getInstance().getMailSender().send(mimeMessage);
                } catch (Exception e) {
