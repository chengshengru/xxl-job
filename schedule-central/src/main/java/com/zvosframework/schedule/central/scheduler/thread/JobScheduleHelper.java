package com.zvosframework.schedule.central.scheduler.thread;

import com.zvosframework.schedule.central.constant.TriggerStatus;
...
import com.zvosframework.schedule.central.scheduler.misfire.MisfireStrategyEnum;
import com.zvosframework.schedule.central.scheduler.trigger.TriggerTypeEnum;
import com.zvosframework.schedule.central.scheduler.type.ScheduleTypeEnum;
    ...
    public void start(){
        ...
        scheduleThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                logger.info(">>>>>>>>> init schedule-job central scheduler success.");

                // pre-read count: treadpool-size * 10 (trigger-qps: 1000ms / 100ms each trigger cost)
                int preReadCount = (JobCentralBootstrap.getInstance().getTriggerPoolFastMax() + JobCentralBootstrap.getInstance().getTriggerPoolSlowMax()) * 10;
                ...
                while (!scheduleThreadToStop) {
                    ...
                    try {
                        transactionStatus = JobCentralBootstrap.getInstance().getTransactionManager().getTransaction(new DefaultTransactionDefinition());
                        // 1、job lock
                        String lockedRecord = JobCentralBootstrap.getInstance().getJobLockMapper().scheduleLock();
                        long nowTime = System.currentTimeMillis();

                        // scan and process job
                        List<JobInfo> scheduleList = JobCentralBootstrap.getInstance().getJobInfoMapper().scheduleJobQuery(nowTime + PRE_READ_MS, preReadCount);
                        if (CollectionTool.isNotEmpty(scheduleList)) {
                            ...
                            for (JobInfo jobInfo: scheduleList) {
                                ...
                                if (nowTime > jobInfo.getTriggerNextTime() + PRE_READ_MS) {
                                    ...
                                    // 1、trigger direct
                                    JobCentralBootstrap.getInstance().getJobTriggerPoolHelper().trigger(jobInfo.getId(), TriggerTypeEnum.CRON, -1, null, null, null);
                            ...
                            /*for (JobInfo jobInfo: scheduleList) {
                                JobCentralBootstrap.getInstance().getJobInfoMapper().scheduleUpdate(jobInfo);
                            }*/
                            int batchSize = JobCentralBootstrap.getInstance().getScheduleBatchSize();
                            ...
                            for (List<JobInfo> scheduleListBatch : scheduleListBatches) {
                                int totalAffected = JobCentralBootstrap.getInstance().getJobInfoMapper().scheduleBatchUpdate(scheduleListBatch);
                        ...
                        try {
                            if (transactionStatus != null) {
                                JobCentralBootstrap.getInstance().getTransactionManager().commit(transactionStatus);   // avlid schedule repeat
                            }
                        } catch (Throwable e) {
        ...
        scheduleThread.setName("schedule-job, central JobScheduleHelper#scheduleThread");
                    ...
                    try {
                        ...
                        if (CollectionTool.isNotEmpty(ringItemData)) {
                            ...
                            for (int jobId: ringItemData) {
                                // do trigger
                                JobCentralBootstrap.getInstance().getJobTriggerPoolHelper().trigger(jobId, TriggerTypeEnum.CRON, -1, null, null, null);
                            }
                            // clear
                            ringItemData.clear();
                        }
        ...
        ringThread.setName("schedule-job, central JobScheduleHelper#ringThread");
        ringThread.start();
    }
