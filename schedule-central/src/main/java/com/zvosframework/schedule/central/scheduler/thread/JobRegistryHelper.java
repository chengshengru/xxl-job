...
public class JobRegistryHelper {
	...
	public void start(){
		...
		registryOrRemoveThreadPool = new ThreadPoolExecutor(
				...
				new ThreadFactory() {
					...
					public Thread newThread(Runnable r) {
						return new Thread(r, "schedule-job, central JobRegistryMonitorHelper-registryOrRemoveThreadPool-" + r.hashCode());
					}
				},
		...
		registryMonitorThread = new Thread(new Runnable() {
			...
			public void run() {
				while (!toStop) {
					try {
						// auto registry group
						List<JobGroup> groupList = JobCentralBootstrap.getInstance().getJobGroupMapper().findByAddressType(0);
						if (groupList!=null && !groupList.isEmpty()) {

							// remove dead address (admin/executor)
							List<Integer> ids = JobCentralBootstrap.getInstance().getJobRegistryMapper().findDead(Const.DEAD_TIMEOUT, new Date());
							if (ids!=null && !ids.isEmpty()) {
								JobCentralBootstrap.getInstance().getJobRegistryMapper().removeDead(ids);
							}

							// fresh online address (admin/executor)
							HashMap<String, List<String>> appAddressMap = new HashMap<String, List<String>>();
							List<JobRegistry> list = JobCentralBootstrap.getInstance().getJobRegistryMapper().findAll(Const.DEAD_TIMEOUT, new Date());
							if (list != null) {
							...
							for (JobGroup group: groupList) {
								...
								JobCentralBootstrap.getInstance().getJobGroupMapper().update(group);
							}
						}
					} catch (Throwable e) {
						if (!toStop) {
		...
		registryMonitorThread.setName("schedule-job, central JobRegistryMonitorHelper-registryMonitorThread");
	...
	public Response<String> registry(RegistryRequest registryParam) {
		...
		registryOrRemoveThreadPool.execute(new Runnable() {
			...
			public void run() {
				...
				int ret = JobCentralBootstrap.getInstance().getJobRegistryMapper().registrySaveOrUpdate(registryParam.getRegistryGroup(), registryParam.getRegistryKey(), registryParam.getRegistryValue(), new Date());
				if (ret == 1) {
					// fresh (add)
	...
	public Response<String> registryRemove(RegistryRequest registryParam) {
		...
		registryOrRemoveThreadPool.execute(new Runnable() {
			@Override
			public void run() {
				int ret = JobCentralBootstrap.getInstance().getJobRegistryMapper().registryDelete(registryParam.getRegistryGroup(), registryParam.getRegistryKey(), registryParam.getRegistryValue());
				if (ret > 0) {
					// fresh (delete)
					freshGroupRegistryInfo(registryParam);
				}
