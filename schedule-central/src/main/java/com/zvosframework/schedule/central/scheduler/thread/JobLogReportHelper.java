...
public class JobLogReportHelper {
    ...
    public void start(){
                ...
                while (!toStop) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < 3; i++) {
                            ...
                            // fill count-data
                            Map<String, Object> triggerCountMap = JobCentralBootstrap.getInstance().getJobLogMapper().findLogReport(todayFrom, todayTo);
                            ...
                            // do refresh:
                            JobCentralBootstrap.getInstance().getJobLogReportMapper().saveOrUpdate(xxlJobLogReport);      // 0-fail; 1-save suc; 2-update suc;
                            /*if (ret < 1) {
                                JobCentralBootstrap.getInstance().getJobLogReportMapper().save(xxlJobLogReport);
                            }*/
                        }
                    ...
                    try {
                        if (JobCentralBootstrap.getInstance().getLogretentiondays()>0
                                && System.currentTimeMillis() - lastCleanLogTime > 24*60*60*1000) {

                            // expire-time
                            Calendar expiredDay = Calendar.getInstance();
                            expiredDay.add(Calendar.DAY_OF_MONTH, -1 * JobCentralBootstrap.getInstance().getLogretentiondays());
                            ...
                            do {
                                logIds = JobCentralBootstrap.getInstance().getJobLogMapper().findClearLogIds(0, 0, clearBeforeTime, 0, 1000);
                                if (logIds!=null && !logIds.isEmpty()) {
                                    JobCentralBootstrap.getInstance().getJobLogMapper().clearLog(logIds);
                                }
        ...
        logReportThread.setName("schedule-job, central JobLogReportHelper");
