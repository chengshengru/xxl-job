 ...
 *      3、执行日志：需要通过 "JobHelper.log" 打印执行日志；
 *      4、任务结果：默认任务结果为 "成功" 状态，不需要主动设置；如有诉求，比如设置任务结果为失败，可以通过 "JobHelper.handleFail/handleSuccess" 自主设置任务结果；
 *
...
public class SampleJob {
    ...
    public void commandJobHandler() throws Exception {
        ...
        try {
            ...
            if (command==null || command.trim().length()==0) {
                JobHelper.handleFail("command empty.");
                return;
            }
    ...
    public void httpJobHandler() throws Exception {
        ...
        try {
            ...
            JobHelper.log(new RuntimeException("HttpJobParam parse error", e));
            JobHelper.handleFail();
            return;
        }
        ...
        if (httpJobParam == null) {
            JobHelper.log("param parse fail.");
            JobHelper.handleFail();
            return;
        }
        if (StringTool.isBlank(httpJobParam.getUrl())) {
            JobHelper.log("url["+ httpJobParam.getUrl() +"] invalid.");
            JobHelper.handleFail();
            return;
        }
        if (!isValidDomain(httpJobParam.getUrl())) {
            JobHelper.log("url["+ httpJobParam.getUrl() +"] not allowed.");
            JobHelper.handleFail();
            return;
        }
        ...
        if (StringTool.isNotBlank(httpJobParam.getMethod())) {
            ...
            if (methodParam == null) {
                JobHelper.log("method["+ httpJobParam.getMethod() +"] invalid.");
                JobHelper.handleFail();
                return;
            }
            method = methodParam;
        ...
        try {
            ...
            JobHelper.log("StatusCode: " + httpResponse.statusCode());
            JobHelper.log("Response: <br>" + httpResponse.response());
        } catch (Exception e) {
            JobHelper.log(e);
            JobHelper.handleFail();
        }
